...
router.get('/operations', requireAuth, async (req, res) => {
  try {
    const { assetId, assetCode } = req.query

    if ((!assetId && !assetCode) || (assetId && typeof assetId !== 'string') || (assetCode && typeof assetCode !== 'string')) {
      return res.status(400).json({ error: 'Asset ID or Asset Code is required' })
    }

    const portfolioAssets = await getCollection('portfolio_assets')
    const userId = new ObjectId(req.user.userId)

    // Buscar ativo por ID ou código
    let asset
    if (assetId) {
      asset = await portfolioAssets.findOne({
...
router.post('/operations', requireAuth, async (req, res) => {
  try {
    const { assetId, assetCode, tipo_operacao, quantidade, preco, data_operacao, notes } = req.body

    console.log('POST /operations - Request body:', { assetId, assetCode, tipo_operacao, quantidade, preco, data_operacao })

    if ((!assetId && !assetCode) || !tipo_operacao || quantidade === undefined || preco === undefined) {
      return res.status(400).json({ error: 'Missing required fields (need assetId or assetCode)' })
    }
    ...
    // Se o ativo não existir e foi fornecido assetCode, criar automaticamente
    if (!asset && assetCode) {
      console.log(`Asset ${assetCode} not found, creating automatically`)
      const newAsset = {
        userId,
        assetCode: assetCode.toUpperCase(),
        currentPosition: {
          ...
          averagePrice: 0,
          totalInvested: 0,
          firstPurchaseDate: data_operacao ? new Date(data_operacao) : new Date(),
        },
        operations: [],
        createdAt: new Date(),
        updatedAt: new Date(),
      ...
      console.log(`Asset created with ID: ${result.insertedId}`)
    }

    if (!asset) {
      return res.status(404).json({ error: 'Asset not found. Please provide assetCode or create the asset first.' })
    }
    ...
    const updatedAsset = result
    let totalQuantity = 0
    let totalInvested = 0
    let firstDate = updatedAsset.currentPosition?.firstPurchaseDate

    updatedAsset.operations.forEach(op => {
      if (op.type === 'entrada') {
